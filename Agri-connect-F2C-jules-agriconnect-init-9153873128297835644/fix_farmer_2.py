...
block_pattern = r'<div className="flex gap-2">[\s\S]*?Mark Delivered[\s\S]*?<\/button>\s*\}\)\}\s*<\/div>'
...
code = re.sub(block_pattern, replacement, code)

# Fix Address
address_pattern = r'<p className="flex items-start"><MapPin className="w-4 h-4 mr-1 mt-0\.5 flex-shrink-0" \/> \{o\.delivery_address\?\.address_line \|\| "Address not provided"\}<\/p>'
address_replacement = """<div className="flex items-start">
...
code = re.sub(address_pattern, address_replacement, code)
