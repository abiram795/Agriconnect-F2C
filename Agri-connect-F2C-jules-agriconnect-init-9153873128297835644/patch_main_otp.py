...
old_status = """@app.patch("/api/orders/{order_id}/farmer-status")
...
async def update_farmer_order_status(order_id: UUID, status_update: FarmerOrderStatusUpdate):
    ...
    async with httpx.AsyncClient() as client:
        ...
        # If order is being confirmed/accepted, generate OTP
        # or if it transitions to Awaiting Delivery OTP
        # We will generate OTP if it doesn't have one and is moving past pending
        if status_update.status in ['Confirmed', 'Farmer Accepted', 'Preparing', 'Ready for Pickup', 'Out for Delivery']:
            if not order_data.get("delivery_otp_hash"):
                ...
                # To securely pass back to consumer, we just store it. Consumer retrieves via their endpoint.
...
async def generate_new_otp(order_id: UUID):
    # This is for the consumer to generate/view their OTP
    # We will generate a new one, return it to the frontend, and store the hash
    async with httpx.AsyncClient() as client:
        headers = get_supabase_headers()
        res = await client.get(f"{supabase_url}/rest/v1/orders?id=eq.{order_id}", headers=headers)
        if res.status_code != 200 or not res.json():
...
if old_status in code:
    ...
    with open("backend/main.py", "w", encoding="utf-8") as f:
        f.write(code)
    print("OTP Routes updated successfully!")
else:
    print("Could not find old status block.")
